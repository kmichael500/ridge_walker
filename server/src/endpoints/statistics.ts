...
statisticsAPI.get('/', (req, res, next) => {
  const getStats = async function () {
    const mapped = await MasterPoint.countDocuments({
      'properties.map_status': 'Mapped',
    });
    // const notGrade5 =  await MasterPoint.countDocuments({'properties.map_status':{"$ne":/^(Mapped|Unmapped|$^)$/g}});
    const sketch = await MasterPoint.countDocuments({
      'properties.map_status': 'Sketch',
    });
    const paceCompass = await MasterPoint.countDocuments({
      'properties.map_status': /^Pace & Compass$/g,
    });
    const tapeCompass = await MasterPoint.countDocuments({
      'properties.map_status': /^Tape & Compass$/g,
    });
    const unmapped = await MasterPoint.countDocuments({
      'properties.map_status': 'Unmapped',
    });
    ...
    for (const key in sortedCaves) {
      ...
      if (longestByCounty[sortedCaves[key].properties.co_name] === undefined) {
        ...
        longestByCounty[sortedCaves[key].properties.co_name].tenLongest.length <
        10
      ) {
        longestByCounty[sortedCaves[key].properties.co_name].tenLongest.push(
          sortedCaves[key].properties.length
        );
    ...
    const extraEnteranceTotal = await MasterPoint.countDocuments({
      'properties.tcsnumber': /^\D+\w+E\d+$/g,
    });
    const totalCaves = await MasterPoint.countDocuments({
      'properties.tcsnumber': /^\D+\d+$/g,
    });
    res.json({
      mapStats,
      totalPoints,
...
statisticsAPI.get('/county', (req, res, next) => {
  ...
  const geojson = GeoJSON.parse(county_center, {
