...
parcelAPI.get('/:coords', async (req, res, next) => {
    ...
    try{
        const lookupData = await axiosInstance.get('https://landgrid.com/search.json?query=' + req.params.coords);
        const path = lookupData.data[0].path.split('/');
        const county = path[3];
        const city = path[4];
        const parcel = path[5];
        parcelDataURL = "https://landgrid.com/us/tn/"+county+"/"+city+"/" + parcel+".json";
    }
    catch(error){
        // console.log(error);
        res.send(error);
